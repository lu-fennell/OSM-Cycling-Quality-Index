...
class Trace:
    ...
    def add_layer(self, layer: QgsVectorLayer, tag: str = None):
        fname = write_layer(self.out_dir, self._next_name(tag), layer)
        if self.pretty:
            _make_pretty(fname)

    def add_dict(self, d: dict, tag: str = None):
        write_dict(self.out_dir, self._next_name(tag), d, self.pretty)
        
            

    def _next_name(self, tag: str = None) -> str:
        if tag:
            name = f'{self.count}_{tag}'
        else:
            name = f'{self.count}'
        self.count = self.count + 1
        return name

def write_dict(out_dir: str, trace_item_name: str, d: dict, pretty: bool = False) -> str:
